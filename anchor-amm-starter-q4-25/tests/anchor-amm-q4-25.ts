...
describe("anchor-amm-q4-25", () => {
  ...
  const seed = new anchor.BN(2025);
  const fee = 25; // 0.25%
  const initialAmount = 30_000_000_000;
  ...
  before(async () => {
    ...
    console.log("User X ATA:", userX.toBase58());
    console.log("User Y ATA:", userY.toBase58());
    console.log("Config PDA:", configPda.toBase58());
    console.log("Mint LP PDA:", mintLp.toBase58());
    console.log("Vault X PDA:", vaultX.toBase58());
    console.log("Vault Y PDA:", vaultY.toBase58());
    console.log("User LP PDA:", userLp.toBase58());
  });
  ...
  it("initialize pool", async () => {
    ...
    console.log("pool initialized, tx is: ", tx);
  ...
  it("Initial Deposit to Liquidity", async () => {
    ...
    console.log(
      `In this case depositMaxX is: ${BigInt(depositMaxX.toString()) / SIX_DECIMAL
      }`
    );
    console.log(
      `In this case depositMaxY is: ${BigInt(depositMaxY.toString()) / SIX_DECIMAL
      }`
    );
    console.log(
      `In this case amount to be deposit is: ${BigInt(1_000_000_000) / SIX_DECIMAL
      }`
    );
    const tx = await program.methods
    ...
    console.log(
      `Vault X balance: ${BigInt(vaultXAccount.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );
    console.log(
      `Vault Y balance: ${BigInt(vaultYAccount.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );
    console.log(
      `User LP balance: ${BigInt(userLpAccountInfo.amount.toString()) / SIX_DECIMAL
      } LP tokens`
    );
  ...
  it("Second deposit to Liquidity", async () => {
    ...
    console.log(
      `In this case amount to be deposit is: ${BigInt(100_000_000) / SIX_DECIMAL
      }`
    );
  ...
  it("Swap 100 token X for token Y", async () => {
    ...
    console.log(
      `User have Initial X token: ${BigInt(userXAmountBefore.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );
    console.log(
      `User have Initial Y token: ${BigInt(userYAmountBefore.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );
    console.log(
      `User have After X token: ${BigInt(userXAmountAfter.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );
    console.log(
      `User have After Y token: ${BigInt(userYAmountAfter.amount.toString()) / SIX_DECIMAL
      } Tokens`
    );

    console.log(
      `X balance change: ${BigInt(
        (userXAmountBefore.amount - userXAmountAfter.amount).toString()
      ) / SIX_DECIMAL
      }`
    );
    console.log(
      `Y balance change: ${BigInt(
  ...
  it("Withdraws liquidity from the pool", async () => {
    ...
    console.log(
      `LP tokens burned: ${BigInt((userLpBefore.amount - userLpAfter.amount).toString()) /
      SIX_DECIMAL
      }`
    );
    console.log(
      `X received: ${BigInt((userXAfter.amount - userXBefore.amount).toString()) /
      SIX_DECIMAL
      }`
    );
    console.log(
      `Y received:", ${BigInt((userYAfter.amount - userYBefore.amount).toString()) /
      SIX_DECIMAL
      }`
    );
  });
